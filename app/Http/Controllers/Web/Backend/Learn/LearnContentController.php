...
namespace App\Http\Controllers\Web\Backend\Learn;
...
class LearnContentController extends Controller
{
    public function index(Request $request): View|JsonResponse
    {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    return '<div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                <a href="' . route('learn.content.edit', ['id' => $data->id]) . '" type="button" class="text-white btn btn-primary fs-14 edit-icn" title="Edit">
        ...
        return view("backend.layouts.learn.index");
    }
    ...
    public function create()
    {
        ...
        // dd($categories);
        return view("backend.layouts.learn.create", compact('categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('thumbnail')) {
            $imagePath = Helper::fileUpload($request->file('thumbnail'), 'learn', time() . '_' . $request->file('thumbnail')->getClientOriginalName());
        ...
        return redirect()->route('learn.content.list')->with('t-success', 'Learn Content created successfully.');
    }

    public function edit(int $id)
    {
        $learn = Learn::where('id', $id)->firstOrFail();
        $categories = Categories::where('type', 'Learn')->where('status', 'active')->get();
        return view('backend.layouts.learn.edit', compact('learn', 'categories'));
    }
    ...
    public function update(Request $request, int $id)
    {
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            if ($imagePath !== null) {
                ...
                Helper::fileDelete($learn->thumbnail);
                $learn->thumbnail = $imagePath;
            }
        }

        $learn->title = $request->title;
        $learn->description = $request->description;
        $learn->category = $request->category;
        $learn->details = $request->details;
        $learn->vimeo_id = $request->vimeo_id;
        $learn->status = $request->status;

        $learn->update();

        return redirect()->route('learn.content.list')->with('t-success', 'Learn Content updated successfully.');
    }
